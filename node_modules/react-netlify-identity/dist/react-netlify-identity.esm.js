...
function runRoutes(gotrue, setUser, remember) {
  ...
  if (matchesActionHashes) {
    ...
    if (params.confirmation_token) {
      gotrue.confirm(params.confirmation_token).then(setUser)["catch"](console.error); // dont notify dev as this package does not export its own method for this
...
function useNetlifyIdentity(url, onAuthChange, enableRunRoutes) {
  ...
  var goTrueInstance = useMemo(function () {
    return new GoTrue({
  ...
  var _useState = useState(goTrueInstance.currentUser() || undefined),
      user = _useState[0],
      setUser = _useState[1];

  var _setUser = useCallback(function (_user) {
    setUser(_user);
  ...
  var _useState2 = useState(defaultParam),
      param = _useState2[0],
  ...
  var loginProvider = useCallback(function (provider) {
  ...
  var acceptInviteExternalUrl = useCallback(function (provider, autoRedirect) {
  ...
  var signupUser = useCallback(function (email, password, data, directLogin) {
  ...
  var loginUser = useCallback(function (email, password, remember) {
    if (remember === void 0) {
      remember = true;
    }
  ...
  var requestPasswordRecovery = useCallback(function (email) {
  ...
  var recoverAccount = useCallback(function (remember) {
  ...
  var updateUser = useCallback(function (fields) {
    if (!user) {
  ...
  var getFreshJWT = useCallback(function () {
    if (!user) {
      return Promise.reject(errors.noUserFound);
    }
  ...
  var logoutUser = useCallback(function () {
    if (!user) {
      return Promise.reject(errors.noUserFound);
    }
...
export { IdentityContextProvider, useIdentityContext, useNetlifyIdentity };
//# sourceMappingURL=react-netlify-identity.esm.js.map
